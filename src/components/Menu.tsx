...
function Menu({ menuItems }: Props) {
    ...
    return <AnimatePresence>
        {showNav &&
            ...
            >
                <ul className='flex sm:flex-row items-center sm:gap-4 md:gap-6 sm:mt-8 rounded-full px-10 sm:h-16 bg-lime-900 bg-opacity-50 hover:bg-opacity-100 duration-150 text-slate-100'>
                    {menuSections}
                </ul>
            </motion.nav>
        }
